...
export default function Game() {
  ...
  const gameRef = db
    ...
    .collection("players")
    .orderBy("score", "desc");
  const messageRef = db.collection("games").doc(gameID).collection("chat");
  ...
  return (
    <div className="App font-jetbrains bg-gradient-to-tr from-white via-purple-200 to-blue-300 flex w-full lg:w-screen h-full xl:h-screen justify-items-center items-center">
      <div className="grid grid-cols-4 w-full gap-4 my-6 mx-4 md:mx-auto xl:max-w-7xl">
        ...
        >
          <h1 className="sm:text-xl md:text-2xl lg: text-3xl xl:text-6xl font-bold font-jetbrains">
            {title}
          </h1>
        </div>
        ...
        <div
          ...
          className="order-5 lg:order-2 row-span-3 col-span-4 lg:col-span-1 bg-blue-200 border-4 border-pink-500 rounded-3xl p-4 flex flex-col items-center overflow-auto"
        >
          <h2 className="text-lg font-jetbrains font-bold mb-2">
            Leader Board
          </h2>
          {leaderboard &&
            leaderboard.map((player, index) => (
              <div
                key={player.name}
                className="grid grid-cols-5 gap-4 mb-1 bg-white border border-black px-4 rounded-xl"
              >
                <div>
                  <h3 className="font-bold text-2xl mt-3">#{index + 1}</h3>
                </div>
                <div>
                  <img src={player.image} className="h-12" alt="" />
                </div>
                <div className="col-span-3">
                  <h3 className="text-lg font-jetbrains font-bold">
                    {player.name}
                  </h3>
        ...
        <div
          id="artSection"
          className="order-2 lg:order-3 row-span-4 h-[500px] lg:min-h-[600px]  lg:row-span-4 col-span-4 lg:col-span-2  bg-white border-4 border-pink-500 rounded-3xl flex flex-col p-8 justify-items-center items-center"
        ></div>
        ...
        >
          <h2>Chat</h2>
          <div className="overflow-auto w-full h-full mt-auto">
            {messages &&
              messages.map((message) => (
                <div
                  key={message.id}
                  className="mb-1 bg-white border border-black px-4 rounded-xl"
                >
          ...
          <div className="flex mx-2 mt-auto">
            <input
              type="text"
              className="w-full border-2 border-black rounded-lg pl-2"
            />
            <button className="bg-white ml-2 border-2 border-black rounded-lg px-2 py-1 active:bg-blue-400">
              Send
            </button>
          </div>
        </div>
        ...
        >
          ...
          <button
            ...
            className="mt-2 w-[75%] bg-white rounded-lg border-2 py-2 border-black hover:bg-blue-200 active:bg-blue-300 focus:border-blue-600"
          >
            {" "}
            <span className="flex justify-center">
              <img src={copy} className="h-4 mt-1 mr-3" alt="" />
              Copy Link{" "}
            </span>
          </button>
        </div>
